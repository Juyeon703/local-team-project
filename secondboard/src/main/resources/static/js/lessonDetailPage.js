  ...
  Swal.fire({
    ...
    if (confirm.isConfirmed) {
      $.ajax({
        type: "post",
        async: false,
        url: "/api/comments/commentAddLesson/" + comment.noticeId,
        data: comment,
...
const deleteComment = (commentId) => {
  Swal.fire({
    ...
    if (confirm.isConfirmed) {
      $.ajax({
        type: "post",
        url: "/api/comments/commentDeleteLesson/" + commentId,
        success: function (data) {
          if (data.result === "success") {
            window.location.reload();
          }
...
const updateCommentConfirm = (commentId) => {
  ...
  Swal.fire({
    ...
    if (confirm.isConfirmed) {
      $.ajax({
        type: "post",
        url: "/api/comments/commentUpdateLesson/" + commentId,
        data: comment,
        success: function (data) {
          if (data.result === "success") {
            window.location.reload();
...
const updateCommentCancel = (commentId) => {
  ...
  $.ajax({
    type: "post",
    url: "/api/comments/commentUpdateCancelLesson/" + commentId,
    data: comment,
    success: function (data) {
      if (data.result === "success") {
        window.location.reload();
...
const deleteNotice = (noticeId) => {
  Swal.fire({
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.result === "success") {
          window.location.assign("localhost:8080/lesson/home");
        }
      },
      error: function () {
        Swal.fire({
